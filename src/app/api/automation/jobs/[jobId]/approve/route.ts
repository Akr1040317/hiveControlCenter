...
export async function POST(request: Request, context: RouteParams) {
  try {
    assertRateLimit(request, "automation-jobs-approve", {
      limit: 100,
      windowMs: 60_000,
    });
  ...
  try {
    assertPermission(session, "automation.approve");
  } catch {
  ...
  try {
    ...
    await logAdminAction({
      actor: session,
      actionType: "automation.job.approve",
    ...
    await logAdminAction({
      ...
      requestPayloadRedacted: { reason: parsed.data.reason },
      result: "failure",
      errorCode: "AUTOMATION_APPROVE_FAILED",
